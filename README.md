...
**也能自动带出头像 + 昵称（主页有公开 OG 信息时）：**
小红书、抖音、TikTok、微信、QQ、站酷、网易云音乐、LOFTER

> 所有平台都会先尝试结构化接口，失败（如微博登录风控、抖音接口签名）再自动改用**主页公开 OG 标签生成**昵称与头像，绝不会硬失败。粉丝数抓不到时卡片不显示该项，手动补填即可，外观完全一样。

**完全拿不到时（主页无公开信息）：**
直接手动填。昵称、粉丝数照抄主页数字；头像可在手机 App 保存原图后传到仓库，或右键复制图片地址粘进「头像地址」。
